...
def judge(items: list[dict], *, subjects: list[dict], key: str, base: str,
    ...
    prompt = (
        ...
        "from the subject list. For discovery_scope=music, anime-series membership is NOT an "
        "approval criterion: no subject_slug is correct for an unrelated artist. Never choose "
        "MANUAL or OUT_OF_SCOPE solely because that artist is absent from the subject catalog. "
        "A Japanese physical concert can be in scope without a listed anime subject, but an album, song, MV, stream or overseas show "
        ...
        "detail link. Judge only ticket milestones the candidate actually claims: an unannounced "
        "closing time is not a contradiction when the candidate stores only a supported opening time. "
        "If claimed facts or round-to-session mappings are unclear, MANUAL. "
