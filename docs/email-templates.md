...
`services/product/src/genchi_product/emails.py` 是不访问网络或数据库的渲染层，返回 `subject`、`text` 和 `html`。HTML 源文件位于 `templates/login_code.html`，通过 Python package data 随安装包分发。登录流程传入实际生成的六位验证码、实际有效分钟数和 `PUBLIC_SITE_URL`；模板不保存验证码，不在主题、日志或对象 repr 中暴露验证码。
...
SMTP 以 `multipart/alternative` 投递，先放 UTF-8 纯文本，再放 HTML。不支持 HTML 的客户端可直接阅读纯文本。认证邮件不添加营销退订头；现有活动通知继续使用原来的发送路径和正文。
