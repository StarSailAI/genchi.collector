...
实现与验收结果完成后记录在 docs/archive/foundation/acceptance.md。
...
FoundationServices 的 API Key 模块没有可采用 Recipe，因此 `foundation.project.yaml` 不将其声明为已启用 Foundation 模块。项目自建实现及偏离原因记录在 `docs/architecture/foundation-decisions.md`，接口和运维边界记录在 `docs/reference/agent-api.md`。
