...
当前 Schema 1.11 增加艺人主体类型；首页 AI 问答与每日倒计时从 Schema 1.10 引入，规则、模型配置和发布验收见 [首页问答](../guides/home-assistant.md)。
...
主体可以有父子关系，例如偶像大师 → 学园偶像大师；关注父系列可选择包含子系列。`subject_type` 区分作品系列与艺人，艺人也可以作为关注目标，活动关联需附带出演证据。活动类型支持演出、音乐节、快闪、咖啡、展览、见面会、物贩和其他。
...
2. ASOBI / e+ / ぴあ / Lawson 的结构化适配器产物先经过时间、类型与主体校验，再与其他来源一样进入批量 AI 二审；不能直接绕过审核显示。
3. 其他官网与官推正文经过大模型抽取；每项活动及节点必须携带正文中的精确证据片段。第一次模型输出只作为待审核候选，不直接修改已发布活动。
4. [批量 AI 审核](../guides/batch-review.md)对通用抽取候选作独立二次核对。官方正文、票务平台的原生场次，以及有可追溯活动链接的编辑来源在通过各自证据门槛后自动发布；明确不属于收录范围的候选可退出队列。证据不足或有冲突的候选仍由管理员在 `/zh-Hans/admin` 核对、修订或拒绝。原文已更新的旧候选不能发布。明确配置的艺人巡演官网使用逐场、逐轮次确定性解析；必须与唯一已发布活动和全部既有场次一一对应，否则整条进入待核，不能猜测合并或发布部分数据。
...
- 多个关注命中同一节点时，只生成一份提醒；优先级为单个活动、系列、关键词、活动类型标签。
...
在 `.env` 中设置 `PRODUCT_SECRET`、`PRODUCT_ADMIN_TOKEN`、前后端一致的 `PRODUCT_PROXY_SECRET`（独立随机值），`PUBLIC_SITE_URL` 与网站 `NEXT_PUBLIC_SITE_URL` 必须一致。默认都为 `http://localhost:13000`。管理员邮箱由 `ADMIN_EMAIL` 指定，本地默认 `admin@genchi.local`。
...
上述读取 API 与参与状态撤销沿用原有表结构。当前完整版本包含名称规范、收信转发、邮箱验证码登录、账户语言、采集日报、Agent API、系列关联证据、首页问答／倒计时与艺人主体类型，SchemaContract 为 1.11。下文的版本号标注各功能首次引入的契约版本。
