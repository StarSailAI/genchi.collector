...
明确配置 `scope:music-offline`、`scope:anime-music-offline` 或 `scope:jpop-offline` 且 `country:JP` 的可信音乐来源，包括 SPICE、LiveFans 等编辑新闻，可收录与动漫系列无关的日本线下演出。新闻中的演出必须有明确的活动名、实体会场、演出日期和可追溯的**外部活动或票务详情链接**，所有候选事实仍需原文证据与 DeepSeek 二审。只有唱片、单曲、MV、直播或海外场次的报道不作为日本线下活动发布；没有精确活动链接等信息的报道继续待核，不能仅凭宽泛艺人页发布。社区来源仍需唯一匹配已核验活动。这些规则只影响活动目录，不创建独立新闻栏目。

ぴあ J-pop 候选另有确定性名称门槛：`formalEventTitle` 必须由同一详情页销售卡片的原文 `titleEvidence` 支持，并且与活动标题一致、不同于 `performerName`。只有艺人页标题而找不到唯一正式演出名时，即使场次和时间完整，仍留待人工审核。二审看到艺人名与售票卡片标题两个不同字段，不能把艺人名当作巡演名。
